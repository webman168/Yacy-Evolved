...
public class AccessTracker_p {
    ...
    public static serverObjects respond(final RequestHeader header, final serverObjects post, final serverSwitch env) {
        ...
        if (page == 0) {
            ...
            long stimeSum = 0;
            long rtimeSum = 0;
            final long utimeSum1 = 0;
            final long stimeSum1 = 0;
            long rtimeSum1 = 0;
            int m = 0;
            ...
            if (page == 2 && AccessTracker.getDumpFile() != null && AccessTracker.getDumpFile().exists()) {
                ...
                final Date fromDate = new Date(toDate.getTime() - 7 * 24 * 3600 * 1000); // 7 Days earlier
                final List<EventTracker.Event> evList = AccessTracker.readLog(AccessTracker.getDumpFile(), fromDate, toDate);
                final OrderedScoreMap<String> topicNavigator = new OrderedScoreMap<String>(String.CASE_INSENSITIVE_ORDER);
                for (final EventTracker.Event ev : evList) {
                    final String qs = ev.payload.toString();
                    if (qs.startsWith("qs ")) { // currently only raw querystring "qs" lines are included
                        final String[] words = CommonPattern.SPACE.split(qs.substring(3));
                ...
                if (topicNavigator == null || topicNavigator.isEmpty()) {
                    ...
                    String name;
                    int i = 0;
                    final int maxcount = topicNavigator.getMaxScore();
                    final Iterator<String> navigatorIterator = topicNavigator.iterator();
